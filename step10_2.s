...
* (1) 質問を表示
    ...
    move.l #0, %D1        /* ch = 0 */
    move.l #Q_MSG, %D2    /* p = #Q_MSG (質問) */
    move.l #34, %D3       /* size = 34 (Q_LEN) */
    trap #0

* (2) 回答を BUF に取得 (入力完了まで待機)
    move.l #SYSCALL_NUM_GETSTRING, %D0
    move.l #0, %D1        /* ch = 0 */
    move.l #BUF, %D2      /* p = #BUF */
    move.l #256, %D3      /* size = 256 (Y/N + Enterで十分) */
    trap #0

* (3) 回答を判定 (BUFの1文字目を見る)
    movea.l #BUF, %A0     /* A0 = BUFのアドレス */
    move.b (%A0), %D4     /* D4 = BUFの最初の1文字 */

    cmpi.b #'Y', %D4       /* 大文字の 'Y' か？ */
    beq .SHOW_RESULT_Y
    cmpi.b #'y', %D4       /* 小文字の 'y' か？ */
    beq .SHOW_RESULT_Y

* (4) Y/y 以外 (N, n, またはその他) の結果を表示
.SHOW_RESULT_N:
    move.l #SYSCALL_NUM_PUTSTRING, %D0
    move.l #0, %D1
    move.l #A_MSG_N, %D2  /* p = #A_MSG_N (回答N) */
    move.l #29, %D3       /* size = 29 (A_LEN_N) */
    trap #0
    bra .HALT_LOOP
...
.SHOW_RESULT_Y:
    move.l #SYSCALL_NUM_PUTSTRING, %D0
    move.l #0, %D1
    move.l #A_MSG_Y, %D2  /* p = #A_MSG_Y (回答Y) */
    move.l #31, %D3       /* size = 31 (A_LEN_Y) */
    trap #0
    bra .HALT_LOOP
...
* --- 課題2用 追加データ ---
Q_MSG:   .ascii "Are you a morning person? (Y/N)\r\n"
.even   /* 奇数長(34B)のため */
Q_LEN:   .dc.l 34
A_MSG_Y: .ascii "Result: You are an early bird!\r\n"
.even   /* 奇数長(31B)のため */
A_LEN_Y: .dc.l 31
A_MSG_N: .ascii "Result: You are a night owl!\r\n"
.even   /* 奇数長(29B)のため */
A_LEN_N: .dc.l 29
.even
* --- 課題2用 追加データ 終了 ---
...
GETSTRING:
	...
	move.l	#0,%d4					/* (2) sz <- 0 (読み込んだ文字数) */
	move.l	%d2,%a0					/* i <- p (バッファポインタ) */

.L_GETSTRING_POLL:
	/* (A) 受信キューから文字が取得できるまで待機 (ポーリング) */
	move.l	#0,%d0					/* ch=0 (受信キュー) */
	jsr	OUTQ					/* (4) OUTQ(0,data) */
	cmp.l	#0,%d0					/* (5) OUTQの復帰値(%d0)をチェック */
	beq	.L_GETSTRING_POLL		/* 失敗(0)なら、再度ポーリング (★修正点：待機する) */
	...
	/* (B) 取得した文字が改行(CR: 0x0D)かチェック (★追加点) */
	cmpi.b	#0x0D, %d1				/* 0x0D = Carriage Return (Enterキー) */
	beq	GETSTRING3				/* 改行なら (9) へ (ループ終了) */
	
	/* (C) バッファサイズチェック */
	cmp.l	%d3,%d4					/* (3) sz = size なら (9) へ */
	beq	GETSTRING3				/* バッファ満杯なら終了 */
	
	/* (8) (A) のポーリングへ戻る */
	bra	.L_GETSTRING_POLL

GETSTRING3:
	move.l	%d4,%d0					/* (9) %d0 <- sz (戻り値セット) */
	bra	GETSTRING_Finish
	
GETSTRING_END:
	move.l	#0,%d0					/* ch=0 以外の場合の戻り値 */
